...
extension PropertyAPIClient {
    ...
    func createAsset(
        ...
        let unit: String
        switch proposedMeterType {
        case "runtime_hours": unit = "hours"
        case "mileage": unit = "miles"
        case "cycles": unit = "cycles"
        default: unit = ""
        }
        request.httpBody = try JSONSerialization.data(withJSONObject: [
            "external_id": "mac-dev-\(UUID().uuidString.lowercased())",
            "name": name,
            "category": category,
            "manufacturer": manufacturer,
            "model": model,
            "location": location,
            "meter_proposed_type": proposedMeterType,
            "meter_proposed_unit": unit,
        ])
    ...
    func renameAsset(id: UUID, name: String) async throws -> MacRanchAsset {
        let url = try makeURL("v1/assets/\(id.uuidString)")
        var request = URLRequest(url: url)
