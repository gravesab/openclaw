-- Unapplied DEV-only Ranch OS livestock lifecycle idempotency outcome contract.
-- Never apply this file to Production.
-- Apply only on an isolated disposable DEV database as ranchos_dev_migrator after 001, 002, and 003.
...
BEGIN
    ...
    IF to_regclass('ranchos.livestock_animals') IS NULL
       ...
       OR to_regclass('ranchos.livestock_lifecycle_events') IS NULL
    ...
    IF NOT EXISTS (
           ...
           WHERE n.nspname = 'ranchos'
             AND c.relname = 'livestock_idempotency'
             AND a.attname = 'result_identifier_id'
             AND NOT a.attisdropped
       )
       OR NOT EXISTS (
           SELECT 1
           ...
           WHERE n.nspname = 'ranchos'
             AND c.relname = 'livestock_idempotency'
             AND a.attname = 'result_retirement_id'
             AND NOT a.attisdropped
       )
       OR NOT EXISTS (
           SELECT 1
       ...
       OR NOT EXISTS (
        ...
        RAISE EXCEPTION '003 identifier idempotency outcomes are required before this migration';
    END IF;
...
ALTER TABLE ranchos.livestock_idempotency
    DROP CONSTRAINT livestock_idempotency_result_matches_operation;

ALTER TABLE ranchos.livestock_idempotency
    ADD COLUMN result_lifecycle_event_id uuid,
    ADD CONSTRAINT livestock_idempotency_committed_lifecycle_event_matches_tenant
        FOREIGN KEY (tenant_id, result_lifecycle_event_id)
    ...
    ADD CONSTRAINT livestock_idempotency_result_matches_operation CHECK (
        (
            ...
            AND result_lifecycle_event_id IS NULL
        ) OR (
            outcome = 'committed'
            ...
            outcome = 'committed'
            AND operation = 'ranchos.livestock.lifecycle-record'
            AND result_animal_id IS NULL
            AND result_identifier_id IS NULL
            AND result_retirement_id IS NULL
            AND result_lifecycle_event_id IS NOT NULL
        ) OR (
            outcome = 'committed'
            AND operation = 'ranchos.livestock.lifecycle-correct'
            AND result_animal_id IS NULL
            AND result_identifier_id IS NULL
            AND result_retirement_id IS NULL
            AND result_lifecycle_event_id IS NOT NULL
...
BEGIN
    ...
    IF NEW.operation = 'ranchos.livestock.animal-create' THEN
        IF NEW.result_animal_id IS NULL
           ...
           OR NEW.result_retirement_id IS NOT NULL
           OR NEW.result_lifecycle_event_id IS NOT NULL THEN
            RAISE EXCEPTION 'idempotency finalize is invalid'
                USING ERRCODE = '23514';
        END IF;
        ...
        IF NEW.result_retirement_id IS NULL
           ...
           OR NEW.result_lifecycle_event_id IS NOT NULL THEN
            RAISE EXCEPTION 'idempotency finalize is invalid'
                USING ERRCODE = '23514';
        END IF;
    ELSIF NEW.operation = 'ranchos.livestock.lifecycle-record' THEN
        IF NEW.result_lifecycle_event_id IS NULL
           OR NEW.result_animal_id IS NOT NULL
           OR NEW.result_identifier_id IS NOT NULL
           OR NEW.result_retirement_id IS NOT NULL THEN
            RAISE EXCEPTION 'idempotency finalize is invalid'
                USING ERRCODE = '23514';
        END IF;
    ELSIF NEW.operation = 'ranchos.livestock.lifecycle-correct' THEN
        IF NEW.result_lifecycle_event_id IS NULL
           OR NEW.result_animal_id IS NOT NULL
           OR NEW.result_identifier_id IS NOT NULL
           OR NEW.result_retirement_id IS NOT NULL THEN
...
DECLARE
    ...
    lifecycle_fk boolean;
    result_check boolean;
    lifecycle_column boolean;
BEGIN
    SELECT pg_get_userbyid(c.relowner), c.relrowsecurity, c.relforcerowsecurity
    INTO table_owner, rls_enabled, rls_forced
    FROM pg_class c
    ...
    SELECT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'livestock_idempotency_committed_lifecycle_event_matches_tenant'
          AND conrelid = 'ranchos.livestock_idempotency'::regclass
    ) INTO lifecycle_fk;
    SELECT EXISTS (
        SELECT 1
    ...
    SELECT EXISTS (
        ...
        WHERE n.nspname = 'ranchos'
          AND c.relname = 'livestock_idempotency'
          AND a.attname = 'result_lifecycle_event_id'
          AND NOT a.attisdropped
    ) INTO lifecycle_column;
    IF NOT identifier_fk OR NOT retirement_fk OR NOT lifecycle_fk OR NOT result_check OR NOT lifecycle_column THEN
        RAISE EXCEPTION 'lifecycle idempotency outcome contract is incomplete';
    END IF;
END;
$$;
