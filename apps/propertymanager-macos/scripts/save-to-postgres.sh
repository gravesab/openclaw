...
# Remove remote categories that are no longer local (House is never deleted).
local_names = {str(c.get("name") or "").strip().casefold() for c in cats if str(c.get("name") or "").strip()}
remote_cats = req("GET", "/categories")
deleted = 0
for remote in remote_cats:
    name = str(remote.get("name") or "").strip()
    if not name or name.casefold() == "house":
        continue
    ...
    if not cid:
        continue
    req("DELETE", f"/categories/{cid}", {"reassign_to": "House"})
    deleted += 1
    print(f"Deleted remote category {name} ({cid})")
...
for t in tasks:
    payload = {
        ...
        "origin": t.get("origin") or ("manufacturer" if (t.get("sourceManualName") or "").strip() else "owner"),
...
print(f"Saved {saved} tasks to Postgres via {api_base}; deleted {deleted} remote categories")
PY
