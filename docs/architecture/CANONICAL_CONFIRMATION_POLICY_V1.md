...
| ranchos.livestock.animal-create         | First-slice Ranch OS livestock animal create                                  | OpenClaw-authoritative ingress then Ranch OS coordinator; typed command with LivestockFactProvenance       | Required; livestock.animal.write                          | CF-2, fresh; 2-minute trusted-time expiry                     | Canonical request digest scoped by tenant and operation; atomic audit | Automation/offline may propose only; MUST NOT confirm                            | Reject missing, expired, reused, or unbound confirmation; non-UUID principal; standing or Production database. |
| ranchos.livestock.identifier-assign     | First-slice Ranch OS livestock identifier assignment                          | OpenClaw-authoritative ingress then Ranch OS coordinator; typed command with LivestockFactProvenance       | Required; livestock.identifier.write                      | CF-2, fresh; 2-minute trusted-time expiry                     | Canonical request digest scoped by tenant and operation; atomic audit | Automation/offline may propose only; MUST NOT confirm                            | Reject missing, expired, reused, or unbound confirmation; active same-tenant identifier collision.             |
| ranchos.livestock.identifier-retire     | First-slice Ranch OS livestock identifier retirement                          | OpenClaw-authoritative ingress then Ranch OS coordinator; typed retirement with closed reason              | Required; livestock.identifier.write                      | CF-2, fresh; 2-minute trusted-time expiry                     | Canonical request digest scoped by tenant and operation; atomic audit | Automation/offline may propose only; MUST NOT confirm                            | Reject missing, expired, reused, or unbound confirmation; retirement before assignment.                        |
| ranchos.livestock.lifecycle-record      | First-slice Ranch OS routine livestock lifecycle record                       | OpenClaw-authoritative ingress then Ranch OS coordinator; typed intake, tagged, or weight event            | Required; livestock.lifecycle.write                       | CF-2, fresh; 2-minute trusted-time expiry                     | Canonical request digest scoped by tenant and operation; atomic audit | Automation/offline may propose only; MUST NOT confirm                            | Reject missing, expired, reused, or unbound confirmation; out-of-order occurrence time.                        |
| ranchos.livestock.lifecycle-correct     | First-slice Ranch OS livestock lifecycle correction                           | OpenClaw-authoritative ingress then Ranch OS coordinator; same-type supersession with closed reason        | Required; livestock.lifecycle.correct                     | CF-2, fresh; 2-minute trusted-time expiry                     | Canonical request digest scoped by tenant and operation; atomic audit | Automation/offline may propose only; MUST NOT confirm                            | Reject missing, expired, reused, or unbound confirmation; second supersession or consumption.                  |
...
- **CONF-016:** A matrix row describes confirmation policy, not implementation permission. The five `ranchos.livestock.*` first-slice rows register CF-2 only and MUST NOT be read as permission to implement a coordinator, repository, ingress, standing DEV database, device path, or Production apply. Sale, death, transfer, archive, and other unregistered livestock operations remain CF-0 under CONF-012.
