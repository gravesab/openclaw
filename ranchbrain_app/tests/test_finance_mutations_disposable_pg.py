...
class _PersistFailingSession:
    ...
    def insert_idempotency_reservation(self, record) -> bool:
        return self._session.insert_idempotency_reservation(record)
...
class FinanceMutationsDisposablePgTests(unittest.TestCase):
    ...
    def test_runtime_can_lock_immutable_finance_references_without_update_privilege(self) -> None:
        session = self._session()
        session.begin()
        session.set_local(principal_id=PRINCIPAL_A, environment="development", tenant_id=TENANT_A)
        try:
            self.assertIsNone(session.lock_account(TENANT_A, "missing-account"))
            self.assertIsNone(session.lock_artifact(TENANT_A, "missing-artifact"))
            self.assertIsNone(session.lock_activity(TENANT_A, "missing-activity"))
        finally:
            session.rollback()
    ...
    def test_interpretation_journal_must_be_complete_at_commit(self) -> None:
        interpretation = self._post_grocery()
        activity_id = self._insert_unposted_activity(interpretation.journal_entry.lines[1].account_id)
        groceries_id = interpretation.splits[0].destination_account_id
        checking_id = interpretation.journal_entry.lines[1].account_id
    ...
    def test_database_rejects_second_root_posting_for_source_activity(self) -> None:
    ...
    def _insert_unposted_activity(self, checking_id: str) -> str:
        ...
        try:
            ...
            cursor.execute(
                ...
                INSERT INTO ranchos.finance_source_activities (
                    ...
                    provenance_observed_at, created_by_user_id
                ) VALUES (%s, %s, %s, CURRENT_TIMESTAMP, %s, %s, %s, %s, %s, CURRENT_TIMESTAMP, %s)
                """,
                (TENANT_A, activity_id, checking_id, "Additional store", Decimal("-12.00"), "fixture", activity_id, "fixture-v1", USER_A),
            )
            self._inspect_conn.commit()
            return activity_id
        except Exception:
