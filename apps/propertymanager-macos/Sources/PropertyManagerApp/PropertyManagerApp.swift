...
enum AppEnvironment {
    ...
    static var credentialService: String {
        isDevelopment ? "ai.openclaw.propertymanager.macos.dev" : "com.redbudranch.propertymanager"
    }
...
struct MaintenanceTask: Identifiable, Codable, Equatable {
    ...
    /// Nil means the task has never been completed or its history is unknown.
    var lastDone: Date?
    var nextDue: Date
    ...
    mutating func recalculateCalendarDueAfterCompletionIfApplicable() {
        ...
        guard (kind == "calendar" || kind == "both"), let lastDone else { return }
...
final class MaintenanceStore: ObservableObject {
    ...
    private var appSupportFolder: URL {
        ...
        let folderName = AppEnvironment.isDevelopment
            ? "PropertyManagerApp-Development"
            : "PropertyManagerApp-Production"
    ...
    private func writeStagingCSV() throws {
        ...
        for task in tasks where task.isActive {
            let columns = [
                task.area,
                task.item,
                task.lastDone.map(DateHelper.isoDate) ?? "",
    ...
    var filteredTasks: [MaintenanceTask] {
        ...
        if let selectedCategoryFilter {
            result = result.filter { task in
                ...
                let linkedAssetCategoryMatches: Bool
                if let assetId = task.assetId,
                   let assetCategory = store.assets.first(where: { $0.id == assetId })?.category {
                    linkedAssetCategoryMatches = assetCategory.caseInsensitiveCompare(selectedCategoryFilter) == .orderedSame
                } else {
                    linkedAssetCategoryMatches = false
                }
                return taskCategoryMatches || linkedAssetCategoryMatches
            }
        }
    ...
    var body: some View {
        ...
        .safeAreaInset(edge: .top, spacing: 0) {
            HStack {
                ...
                Text(BuildIdentity.versionLabel)
                    .font(.caption)
                    .fontWeight(.bold)
                    .monospacedDigit()
            }
            ...
            .accessibilityLabel(
                AppEnvironment.isDevelopment
                    ? "Property Manager development environment, \(BuildIdentity.versionLabel)"
                    : "Property Manager production environment, \(BuildIdentity.versionLabel)"
            )
        }
...
struct TaskRowView: View {
    ...
    var body: some View {
        ...
        HStack(alignment: .top, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                if let badge = task.meterBadge(assets: assets) {
                    Text(badge)
                        .font(.caption2)
    ...
    var statusText: String {
        ...
        let critical = Calendar.current.startOfDay(
            for: task.lastDone.flatMap {
                Calendar.current.date(byAdding: .day, value: task.criticalDays, to: $0)
            } ?? task.nextDue
        )
...
extension MaintenanceTask {
    ...
    /// Meter scheduling is controlled by the linked, activated asset; category is not authoritative.
    func linkedMeterAsset(assets: [MacRanchAsset]) -> MacRanchAsset? {
        guard let assetId,
              let asset = assets.first(where: { $0.id == assetId })
        else { return nil }
        guard asset.meter?.hasMeter == true, asset.meterActivatedAt != nil else { return nil }
        return asset
    }

    func showsMeterTrigger(assets: [MacRanchAsset]) -> Bool {
        linkedMeterAsset(assets: assets) != nil
    }

    func meterUnitLabel(assets: [MacRanchAsset]) -> String {
        guard let meter = linkedMeterAsset(assets: assets)?.meter else { return "units" }
        let configured = meter.unit.trimmingCharacters(in: .whitespacesAndNewlines)
        if !configured.isEmpty { return configured }
        switch meter.meterType {
        case "runtime_hours": return "hours"
        case "mileage": return "miles"
    ...
    func meterBadge(assets: [MacRanchAsset]) -> String? {
        ...
        if let rem = remainingMeter, rem > 0 {
            return "\(NSDecimalNumber(decimal: rem).stringValue) \(unit) left"
        }
        if let trigger = nextDueMeterValue {
            return "Due at \(NSDecimalNumber(decimal: trigger).stringValue) \(unit)"
        }
        return nil
    }
}
...
struct MeterTriggerEditor: View {
    ...
    private var linkedAsset: MacRanchAsset? { task.linkedMeterAsset(assets: assets) }
    private var unitLabel: String { task.meterUnitLabel(assets: assets) }
    private var meterTitle: String {
        switch linkedAsset?.meter?.meterType {
        case "runtime_hours": return "Run-hours trigger"
        case "mileage": return "Mileage trigger"
        case "cycles": return "Cycle trigger"
        default: return "Meter trigger"
        }
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            Text("Absolute trigger is an operator scheduling decision. Manufacturer interval provenance is preserved on save.")
                ...
                .foregroundStyle(.secondary)
            HStack(spacing: 16) {
                LabeledField(title: "Due when meter reaches (\(unitLabel))") {
                    TextField("e.g. 150", text: $triggerText)
                        .textFieldStyle(.roundedBorder)
                        .frame(maxWidth: 160)
                }
                LabeledField(title: "Repeat every (\(unitLabel), optional)") {
    ...
    private func applyTrigger() {
        ...
        guard task.assetId != nil else {
            errorText = "Link a metered asset first."
            return
        }
        guard task.showsMeterTrigger(assets: assets) else {
            errorText = "The linked asset must have an activated meter."
            return
        }
        ...
        task.meterIntervalUnit = unitLabel
...
struct TaskEditorView: View {
    ...
    var basicInfoCard: some View {
        CardView(title: "Basic Info", icon: "info.circle") {
            VStack(alignment: .leading, spacing: 12) {
                ...
                LabeledField(title: "Linked Asset") {
                    Picker(
                        "Linked Asset",
                        selection: Binding(
                            get: { task.assetId },
                        ...
                        Text("No linked asset").tag(nil as UUID?)
                        ForEach(assets.sorted(by: { $0.name.localizedCaseInsensitiveCompare($1.name) == .orderedAscending })) { asset in
                ...
                Text("Link the task to its asset to enable runtime-hours, mileage, or cycle scheduling.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
    ...
    var scheduleCard: some View {
        CardView(title: "Schedule (Shared with Intel)", icon: "calendar") {
            VStack(alignment: .leading, spacing: 12) {
                    ...
                    VStack(alignment: .leading, spacing: 6) {
                        ...
                        Button("Repair Next Due Date") {
                            if let lastDone = task.lastDone {
                                task.nextDue = task.frequency.nextDue(after: lastDone)
                            }
                        }
                        .fixedSize()
                ...
                if task.showsMeterTrigger(assets: assets) {
                    MeterTriggerEditor(
                        task: $task,
                        assets: assets,
    ...
    func markComplete() {
        ...
        let completionDate = DateHelper.isoDate(task.lastDone ?? Date())
        let historyNote: String
    ...
    init(tasks: [MaintenanceTask]) {
        ...
        for task in tasks where task.isActive {
            ...
            let critical = calendar.startOfDay(
                for: task.lastDone.flatMap {
                    calendar.date(byAdding: .day, value: task.criticalDays, to: $0)
                } ?? task.nextDue
            )
